...
## Concrete Pattern — JPA Entity Identity
...
- `hashCode` returns constant (or class-based) to avoid Set/Map corruption after persist
- `toString` excludes lazy-loaded relations to prevent `LazyInitializationException`
- Constructor params are mutable entity fields; `id` is `val` with default

## Concrete Pattern — Uniqueness Constraints for Idempotent Operations

### The Problem — Application-Only Duplicate Check
```kotlin
@Service
class ReservationService(private val reservationRepository: ReservationRepository) {
    @Transactional
    fun createReservation(variantId: Long, orderId: String, quantity: Int): Reservation {
        // BUG: no check for existing reservation — duplicates possible
        val reservation = Reservation(variantId = variantId, orderId = orderId, quantity = quantity)
        return reservationRepository.save(reservation)
    }
}
```
**Bug**: Two concurrent requests for the same `(variantId, orderId)` both succeed. No database constraint and no application check means duplicate reservations silently accumulate.

### The Fix — Database Constraint + Application Guard
```kotlin
@Entity
@Table(
    name = "reservations",
    uniqueConstraints = [
        UniqueConstraint(columnNames = ["variant_id", "order_id"])  // database enforces uniqueness
    ]
)
class Reservation(
...
class ReservationService(private val reservationRepository: ReservationRepository) {
    @Transactional
    fun createReservation(variantId: Long, orderId: String, quantity: Int): Reservation {
        // Application-level check for clean error message
        reservationRepository.findByVariantIdAndOrderId(variantId, orderId)?.let {
            throw IllegalStateException("Reservation already exists for variant=$variantId, order=$orderId")
        }
        val reservation = Reservation(variantId = variantId, orderId = orderId, quantity = quantity)
        return reservationRepository.save(reservation)
    }
...
### Key rules
- **Database constraint is mandatory** — application checks alone have race conditions between SELECT and INSERT
- **Application check provides clean errors** — without it, user gets raw `DataIntegrityViolationException`
- **Both layers together** = belt and suspenders: application catches the common case, database catches the race
- Add `findByXAndY` repository method for the uniqueness check — Spring Data derives the query automatically
