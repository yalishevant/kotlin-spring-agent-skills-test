...
## Concrete Pattern — Reading a BeanCreationException Chain
...
org.springframework.beans.factory.BeanCreationException:
  ...
  No qualifying bean of type 'com.example.notification.NotificationService' available

Caused by: org.springframework.beans.factory.UnsatisfiedDependencyException:
  Error creating bean with name 'orderService' ... parameter 1 ...
...
### Diagnosis Steps
1. **Ignore the top wrapper** — `BeanCreationException` on `orderController` is a symptom, not the cause
2. **Walk to the deepest `Caused by:`** — `NoSuchBeanDefinitionException` for `NotificationService`
3. **Check why the bean is missing:**
   - Is `NotificationService` annotated with `@Service`/`@Component`?
   - Is its package under the `@SpringBootApplication` scan root?
   - Is it in a separate Gradle module not on the classpath?
   - Is it conditional on a profile (`@Profile("prod")`) that isn't active in tests?
   - Is it conditional on a property (`@ConditionalOnProperty`) that's missing?

### The Fix Decision Tree
```
Missing bean?
├── Not annotated → add @Service/@Component
├── Wrong package → fix scanBasePackages or move class
├── Wrong profile → activate profile or add @Profile for test
├── Separate module → add Gradle dependency
└── Conditional on property → add property to config
```

### Common mistakes
- Fixing the first exception in the chain instead of the deepest `Caused by:`
- Adding `@ComponentScan("com.example")` globally instead of fixing the specific missing bean
- Ignoring `@Profile` or `@ConditionalOnProperty` that silently disable the bean
- Missing that a `@Configuration` class itself failed to create, cascading to all its `@Bean` methods
