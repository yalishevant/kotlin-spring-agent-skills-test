...
## Concrete Pattern — Project Brief Extraction

**Input**: A Kotlin + Spring Boot project with `build.gradle.kts`:
```kotlin
plugins {
    id("org.springframework.boot") version "3.3.0"
    kotlin("jvm") version "2.0.21"
    kotlin("plugin.spring") version "2.0.21"
    // NOTE: kotlin("plugin.jpa") is MISSING
}
dependencies {
...
**Extracted Brief**:
```
Stack: Spring Boot 3.3.0, Kotlin 2.0.21, MVC, JPA/Hibernate, H2
Plugins: plugin.spring ✓, plugin.jpa ✗ (MISSING — JPA entities will fail)
Persistence: Spring Data JPA
Test stack: spring-boot-starter-test (default)
Risk markers:
  - Missing kotlin("plugin.jpa") — JPA entities need no-arg constructor
  - No mockk dependency — tests may use Mockito (not idiomatic Kotlin)
  - No testRuntimeOnly("org.junit.platform:junit-platform-launcher")
  - H2 in main dependencies — unclear if production DB differs
```

**Key Trap**: The most common ingestion miss is `plugin.jpa` absence. Without it, `@Entity` classes get `Cannot instantiate class` at runtime despite compiling fine.
