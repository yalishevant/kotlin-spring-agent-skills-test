...
## Tri-State PATCH Semantics — Concrete Pattern

When a PATCH endpoint must distinguish "field omitted" (keep current value) from "field explicitly set to null" (clear the value), a plain nullable Kotlin DTO cannot express this. The `?:` (Elvis) operator treats both cases identically.
...
### Solution — Optional wrapper for PATCH fields
```kotlin
...
Ensure `jackson-module-kotlin` is registered. Jackson deserializes:
- Missing field → `null` (the outer `Optional?` is null → field omitted)
- `"notes": null` → `Optional.empty()` (the outer is present, inner is empty → explicit null)
- `"notes": "value"` → `Optional.of("value")` (present with value)

### Alternative: `JsonNode` approach
```kotlin
data class OrderPatchRequest(
    // Use @JsonProperty + manual parsing from JsonNode
    @JsonIgnore val rawNode: JsonNode? = null
) {
    fun hasField(name: String): Boolean = rawNode?.has(name) == true
    fun isExplicitNull(name: String): Boolean = hasField(name) && rawNode!!.get(name).isNull
}
```

### Key mistakes to avoid
- Using `?:` (Elvis) for PATCH field application — this silently ignores explicit nulls.
- Not registering `jackson-module-kotlin` — `Optional` deserialization may behave differently.
- Using the same DTO for create (where all fields are required) and patch (where fields are optional).
