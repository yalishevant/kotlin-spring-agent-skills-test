...
## Concrete Pattern — Bounded Context with Explicit API Contract

**Problem**: Two features (Orders and Inventory) share the same database tables and service classes. Every change to one breaks the other.

**Fix**: Separate bounded contexts with explicit contract:
```kotlin
...
sealed interface PlaceOrderResult {
    data class Success(val orderId: Long) : PlaceOrderResult
    data class InsufficientStock(val sku: String, val available: Int) : PlaceOrderResult
    data class ValidationError(val errors: List<String>) : PlaceOrderResult
}

// ── Inventory Context (owns stock truth) ──
package com.example.inventory.api

// Inventory exposes a reservation contract, not raw stock tables
interface StockReservationPort {
    fun reserve(items: List<ReservationItem>): ReservationResult
    fun release(reservationId: String)
}

sealed interface ReservationResult {
    data class Reserved(val reservationId: String, val expiresAt: Instant) : ReservationResult
    data class PartiallyAvailable(val unavailable: List<String>) : ReservationResult
}
```

**Key Decisions**:
- Each context owns its tables — no shared writes
- Communication via explicit port interfaces (can be in-process or HTTP)
- Failure semantics explicit in sealed result types
- Inventory does not know about Orders; Orders depend on Inventory's port
