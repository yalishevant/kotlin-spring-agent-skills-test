...
## Concrete Pattern — Java Service to Kotlin Migration
...
public class OrderService {
    ...
    public OrderDto createOrder(CreateOrderRequest request) {
        ...
        log.info("Created order {} for customer {}", order.getId(), order.getCustomerName());
...
class OrderService(
    ...
    fun createOrder(request: CreateOrderRequest): OrderDto {
        ...
        log.info("Created order {} for customer {}", saved.id, saved.customerName)
...
- `@Slf4j` → explicit `LoggerFactory.getLogger(javaClass)` (no Lombok in Kotlin)
- Constructor injection stays as-is — Kotlin primary constructor with `val` is cleaner
- `BigDecimal.compareTo` → Kotlin operator `>` (Kotlin overloads comparison operators for `Comparable`)
- `new Order()` + setters → named constructor arguments (if entity supports it)
- `@Transactional` preserved — must stay on a proxy-visible public method, not an extension function
- Kept explicit `log.info` with `{}` placeholders — string templates (`"Created order ${saved.id}"`) evaluate eagerly regardless of log level

### Common mistakes
- Converting JPA entity to `data class` — breaks equals/hashCode/proxy behavior
- Moving `@Transactional` method to extension function — exits the proxy, transaction stops working
- Replacing `@Slf4j` with Kotlin string templates in log — eager evaluation wastes CPU at debug level
- Using `!!` for repository `findById` result — prefer `orElseThrow` or `?: throw` with descriptive message
